...
run #+begin_src crontab -e#+end_src enter following command at the end
...
** setup ntfd
change the _~/.config/polybar/config_ module-right to include the weather module. 
** make tmux symlink
 make a symlink of the tmux config  #+begin_src ln -s -f ~/.tmux/.tmux.conf ~/.tmux.conf #+end_src
** NOTES
